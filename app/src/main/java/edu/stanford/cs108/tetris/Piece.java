...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		// YOUR CODE HERE
		TPoint[] pointsNew = new TPoint[body.length];
		for(int i=0; i< body.length; i++) {
			pointsNew[i] = new TPoint(height-body[i].y-1 , body[i].x);			
		}
		return new Piece(pointsNew);
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		for(int i=0; i<other.body.length; i++) {
			if(!contains(other.body[i])) {
				return false;
			}
		}
		return true;
	}
	private boolean contains(TPoint p) {
